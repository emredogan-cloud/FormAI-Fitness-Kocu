...
//   5. Always writes `phase72_image_url_sync.sql` next to the seeds —
...
//   6. Appends skeleton entries to `MEAL_IMAGE_PROMPTS.md` for any
...
//   `supabase_rls_policies.sql` restricts `recipes` UPDATE to
...
const _sqlOut = 'phase72_image_url_sync.sql';
const _promptsDoc = 'MEAL_IMAGE_PROMPTS.md';
...
Future<void> main(List<String> args) async {
  ...
  stdout.writeln('• Found ${localFiles.length} bundled .webp files in $_seedDir/');
  ...
  if (!hasService) {
    ...
    stderr.writeln(
        '  or paste $_sqlOut into the Supabase SQL Editor instead.');
    exit(2);
  }
...
void _writeSqlBatch(List<_Recipe> updates) {
  final buf = StringBuffer()
    ..writeln('-- Phase 72 · auto-generated by lib/scripts/sync_recipes_db.dart')
    ..writeln(
