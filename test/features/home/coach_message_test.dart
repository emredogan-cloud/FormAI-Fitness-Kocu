...
void main() {
  test('goal reached → celebration', () {
    expect(weeklyCoachLine(completed: 3, target: 3, hour: 10),
        contains('tamamladın'));
    expect(weeklyCoachLine(completed: 4, target: 3, hour: 10),
        contains('tamamladın'));
  });

  test('one workout left → push', () {
    expect(weeklyCoachLine(completed: 2, target: 3, hour: 14),
        contains('Bir antrenman kaldı'));
  });

  test('nothing done yet is time-of-day aware', () {
    expect(weeklyCoachLine(completed: 0, target: 3, hour: 8),
        contains('Güne güçlü başla'));
    expect(weeklyCoachLine(completed: 0, target: 3, hour: 20),
        contains('Gün bitmeden'));
    expect(weeklyCoachLine(completed: 0, target: 3, hour: 14),
        contains('ilk hareketi'));
  });

  test('mid-week progress shows the count', () {
    final line = weeklyCoachLine(completed: 1, target: 5, hour: 12);
    ...
    for (final c in [0, 1, 2, 5]) {
      for (final t in [0, 3, 5]) {
        for (final h in [0, 9, 13, 19, 23]) {
          expect(weeklyCoachLine(completed: c, target: t, hour: h).trim(),
              isNotEmpty);
        }
      }
    }
